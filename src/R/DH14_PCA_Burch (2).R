...
df.dh[, 8:22] <- lapply(df.dh[, 8:22], as.numeric)

# Extract critical variables for PCA
df.pca <- df.dh[, c(8:18, 20,22)]
...
dh.pca <- prcomp(df.pca, center = TRUE, scale = TRUE)
...
# Family
fviz_pca_ind(dh.pca, geom = "point",
                  habillage=df.dh$Family, addEllipses=TRUE,
                  ellipse.level= 0.95)

# Lineage Generation names
# Specify shapes is a hard one, hot mess graph
fviz_pca_ind(dh.pca, geom = "point",
                  habillage=df.dh$newtreat2, addEllipses=TRUE,
                  ellipse.level= 0.95)
# Plant number
fviz_pca_ind(dh.pca, geom = "point",
             habillage=df.dh$No.tassel, addEllipses=TRUE,
             ellipse.level= 0.95)

# Lineage alone
fviz_pca_ind(dh.pca, geom = "point",
             habillage=df.dh$lineage, addEllipses=TRUE,
             ellipse.level= 0.95)

# Environmental lineage
fviz_pca_ind(dh.pca, geom = "point",
             habillage=df.dh$env.lin, addEllipses=TRUE,
             ellipse.level= 0.95)
